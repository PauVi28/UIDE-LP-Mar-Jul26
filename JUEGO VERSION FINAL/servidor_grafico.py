...
def jugar_servidor(eleccion):
    global resultado_texto, jugada_servidor, jugada_cliente
    global puntaje_servidor, puntaje_cliente, empates
    if not conectado:
        resultado_texto = "Todavía no hay cliente conectado."
        return
    jugada_servidor = eleccion
    cliente_socket.send(jugada_servidor.encode())
    jugada_cliente = cliente_socket.recv(1024).decode()
    ...
    if resultado == "Gana Jugador 1":
        ...
        resultado_texto = "Ganó el cliente"
        puntaje_cliente = puntaje_cliente + 1
    else:
        resultado_texto = "Empate"
        empates = empates + 1
    cliente_socket.send(resultado_texto.encode())
